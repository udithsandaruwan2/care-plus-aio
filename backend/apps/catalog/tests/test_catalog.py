...
class CatalogSeedTests(TestCase):
    def test_seed_catalog_creates_lkr_packages(self):
        ...
        self.assertGreaterEqual(CarePackage.objects.filter(is_active=True).count(), 3)
        self.assertGreaterEqual(AddOn.objects.filter(is_active=True).count(), 4)
        ...
        # Idempotent
        call_command("seed_catalog")
        self.assertTrue(CarePackage.objects.filter(slug="basic-home-care").exists())
        self.assertEqual(
            CarePackage.objects.filter(slug="basic-home-care").count(),
            1,
        )
...
class CatalogApiTests(APITestCase):
    ...
    def test_filter_packages_by_care_level(self):
        ...
        self.assertTrue(all(row["care_level"] == "advanced" for row in resp.data))
        self.assertIn("advanced-clinical", [row["slug"] for row in resp.data])
    ...
    def test_inactive_packages_hidden(self):
        ...
        resp = self.client.get(self.packages_url)
        self.assertFalse(any(row["slug"] == "basic-home-care" for row in resp.data))
